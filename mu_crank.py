...
def swarmalator_traj(N=32, T=100.0, dt=0.01, omega=None, mu=2.5, v=5.0, phi=0.0, sigma=0.05):
    if omega is None:
    ...
    for step in range(1, nt):
        ...
        delta_r = np.minimum(np.abs(delta_r), 1 - np.abs(delta_r))
...
# Irrational mu=2.5
omega_irr = np.tile([1.0, 1/np.sqrt(2)], 16)
traj_irr = swarmalator_traj(omega=omega_irr, mu=2.5)
S_irr_coup = phase_entropy(traj_irr)
traj_irr_unc = swarmalator_traj(omega=omega_irr, mu=0.0, v=0.0)
S_irr_unc = phase_entropy(traj_irr_unc)
drop_irr = (S_irr_unc - S_irr_coup) / S_irr_unc * 100 if S_irr_unc > 0 else 0
print(f"Irr Drop mu=2.5: {drop_irr:.4f}%")

# Rational mu=2.5
omega_rat = np.tile([1.0, 0.5], 16)
traj_rat = swarmalator_traj(omega=omega_rat, mu=2.5)
...
print(f"Rational Drop mu=2.5: {drop_rat:.4f}%")