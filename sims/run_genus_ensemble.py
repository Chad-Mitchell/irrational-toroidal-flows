...
# Full Ensemble Runner with Filter (g=3 Rerun Stub)
# Ties it all—runs irr/rat pairs, computes MI/LE/dim, logs lifts. Seed 42 for repro.

# TODO: [Paste: kuramoto_genus, knn_mi, total_knn_mi, box_dim, lyapunov_max, diffeological_torus_alpha]

def run_genus_ensemble(N=21, genus=3, T_steps=50, dt=0.01, num_ens=10, K=0.5, irr_scale=1.0, trans_coup=0.05, k=3, num_pairs=10):
    """
    Full g=3 filtered ensemble: Irr vs rat, orientable adj pi/3.
    Returns dict: lifts, mis, les, dims.
    """
    ...
    lifts, irr_mis, rat_mis, irr_les, rat_les, irr_dims, rat_dims = [], [], [], [], [], [], []
    ...
    for ens in range(num_ens):
        ...
        # Rat run (same init for fair comp)
...
# Run ex: results = run_genus_ensemble(N=21, genus=3, num_ens=10)
# print(f"Mean lift: {results['mean_lift']:.1f}% ± {results['std_lift']:.1f}%, Wins: {results['win_rate']:.0f}%")
